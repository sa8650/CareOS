...
import { Building2, Calendar, Clock, User, CheckCircle, MapPin, Info } from 'lucide-react';
...
export default function AppointmentForm({ onSuccess }) {
  ...
  // Keep the current step (and any error banner) in view when moving between steps on a phone
  const scrollToForm = useCallback(() => {
    const el = formRef.current;
    if (!el) return;
    const top = el.getBoundingClientRect().top + window.scrollY - 84; // clear the fixed navbar
    if (Math.abs(window.scrollY - top) > 40) window.scrollTo({ top, behavior: 'smooth' });
  }, []);
  const goStep = useCallback((n) => { setStep(n); requestAnimationFrame(scrollToForm); }, [scrollToForm]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.status === 409 && err.data?.day) {
        ...
        requestAnimationFrame(scrollToForm); // error banner sits at the top of the card
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="appt-form" ref={formRef}>
      ...
      {step === 1 && (
        <div className="appt-step fade-in">
          ...
          <button type="button" className="btn btn-primary btn-lg" disabled={!chamber} onClick={() => goStep(2)}>
            Continue
          </button>
        </div>
      )}
      ...
      {step === 2 && chamber && (
        <div className="appt-step fade-in">
              ...
              <div className="calendar-grid">
                ...
                {cells.map((d, i) => d ? (
                  <button type="button" key={d.date}
                    ...
                    disabled={!d.bookable}
                    title={d.bookable ? 'Available' : statusText(d)}>
          ...
          <div className="appt-actions">
            <button type="button" className="btn btn-secondary" onClick={() => goStep(1)}>Back</button>
            <button type="button" className="btn btn-primary btn-lg" disabled={!selectedDate} onClick={() => goStep(3)}>
              Continue
            </button>
          </div>
        </div>
      ...
      {step === 3 && chamber && selectedDay && (
        <div className="appt-step fade-in">
          ...
          <div className="appt-actions">
            <button type="button" className="btn btn-secondary" onClick={() => goStep(2)}>Back</button>
      ...
      <style>{`
        .appt-form { max-width: 720px; background: white; border: 1px solid var(--color-border); border-radius: var(--radius-xl); padding: 1.5rem; box-shadow: var(--shadow-sm); }
        @media (max-width: 640px) { .appt-form { padding: 1.1rem 0.9rem; border-radius: var(--radius-lg); } }
        ...
        /* Calendar */
        .calendar-panel { background: #f1f5f9; border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: 0.75rem; margin-bottom: 0.75rem; }
        ...
        @media (max-width: 640px) {
          ...
          .chamber-select-grid { grid-template-columns: 1fr; }
          .appt-actions { flex-direction: column-reverse; gap: 0.6rem; }
          .appt-actions .btn { width: 100%; justify-content: center; }
          .appt-step > .btn { width: 100%; justify-content: center; }
          .appt-steps li { font-size: 0.78rem; padding: 0.3rem 0.6rem 0.3rem 0.3rem; }
        }
      `}</style>
    </form>
  );
